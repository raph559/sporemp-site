...
const assetVersion = async name => createHash('sha256').update((await readFile(path.join(root,name),'utf8')).replace(/\r\n/g,'\n')).digest('hex').slice(0,12);
const stylesVersion = await assetVersion('styles.css');
const scriptVersion = await assetVersion('site.js');
...
<div class="development-strip wrap"><span class="tag">${t.strip}</span><p>${t.stripText}</p><a href="./${esc(articlePath(data.news[0]))}">${t.stripLink} ${arrow}</a></div>
