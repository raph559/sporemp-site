...
// A fresh output tree prevents removed artwork or retired pages from being redeployed.
...
const svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="15" fill="#182b39"/><circle cx="32" cy="32" r="20" fill="none" stroke="#f9efe0" stroke-width="3"/><circle cx="32" cy="32" r="8" fill="#ed6845"/><circle cx="49" cy="16" r="4" fill="#f4c970"/></svg>';
...
if (french.releases.map(r=>r.version).join(',') !== english.releases.map(r=>r.version).join(',')) throw new Error('Launcher updates must match between languages.');
if (french.roadmap.map(r=>r.state).join(',') !== english.roadmap.map(r=>r.state).join(',')) throw new Error('Roadmap states must match between languages.');
...
for (const [lang, data] of [['en',english],['fr',french]]) {
  ...
  const nav = c => `<header class="header"><a class="brand" href="${c.prefix}" aria-label="SporeMP, ${t.home}"><span class="brand-icon" aria-hidden="true">◌</span>spore<span>mp</span></a><nav aria-label="${t.navigation}"><a href="${c.prefix}#project">${t.project}</a><a href="${c.prefix}#news" ${c.pagePath.startsWith('news/')?'aria-current="page"':''}>${t.news}</a><a href="${c.prefix}#updates">${t.updates}</a><a href="${c.prefix}#roadmap">${t.future}</a><a href="${c.prefix}#support">${t.support}</a></nav><div class="header-tools"><div class="language-switch" role="group" aria-label="${t.language}"><a href="${c.global}${c.pagePath}" lang="en" hreflang="en" aria-label="${copy.en.languageName}" ${lang==='en'?'aria-current="true"':''}>EN</a><span aria-hidden="true">/</span><a href="${c.global}fr/${c.pagePath}" lang="fr" hreflang="fr" aria-label="${copy.fr.languageName}" ${lang==='fr'?'aria-current="true"':''}>FR</a></div></div></header>`;
  const footer = c => `<footer class="footer wrap"><a class="brand" href="${c.prefix}">spore<span>mp</span></a><p>${t.footer}</p><a href="${c.prefix}#project">${t.backProject}</a><div class="fineprint">${t.rights}<br><a href="https://github.com/raph559/SporeMP" target="_blank" rel="noopener noreferrer">${t.sourceCode} ${arrow}</a></div></footer>`;
  const shell = ({title,description,body,c}) => `<!doctype html>
<html lang="${lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="color-scheme" content="light"><meta name="theme-color" content="#f5f2e9"><title>${esc(title)}</title><meta name="description" content="${esc(description)}"><link rel="canonical" href="${esc(langBase+c.pagePath)}"><link rel="alternate" hreflang="en" href="${esc(base+c.pagePath)}"><link rel="alternate" hreflang="fr" href="${esc(base+'fr/'+c.pagePath)}"><link rel="alternate" hreflang="x-default" href="${esc(base+c.pagePath)}"><meta property="og:type" content="${c.pagePath.startsWith('news/')?'article':'website'}"><meta property="og:locale" content="${t.ogLocale}"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(description)}"><meta property="og:url" content="${esc(langBase+c.pagePath)}"><link rel="icon" type="image/svg+xml" href="${favicon}"><link rel="stylesheet" href="${c.global}styles.css"></head><body><a class="skip-link" href="#main-content">${t.skip}</a>${body}<script src="${c.global}site.js" defer></script></body></html>`;
  const c = context();
  const newsCards = data.news.map((n,i)=>`<article class="news-card ${i===0?'featured':''}"><div class="news-meta"><span>${i===0?esc(t.latest):esc(n.category)}</span><time datetime="${n.date}">${date(n.date)}</time></div><h3><a href="./${articlePath(n)}">${esc(n.title)}</a></h3><p>${esc(n.summary)}</p><a class="text-link" href="./${articlePath(n)}" aria-label="${t.readArticle} ${esc(n.title)}">${t.readJournal} ${arrow}</a></article>`).join('');
  ...
  const roadmap = data.roadmap.map((r,i)=>{if(!['done','next','planned'].includes(r.state)) throw new Error('Invalid roadmap state');return `<li class="roadmap-step ${r.state}"><span class="step-number">${String(i+1).padStart(2,'0')}</span><div><span class="tag">${esc(r.status)}</span><h3>${esc(r.title)}</h3><p>${esc(r.description)}</p></div></li>`;}).join('');
...
<section class="hero" aria-labelledby="hero-title"><div class="hero-content wrap"><div class="hero-copy"><p class="eyebrow"><span class="status-dot"></span>${t.heroEyebrow}</p><h1 id="hero-title">${t.heroTitle}</h1><p class="hero-description">${t.heroDescription}</p><div class="hero-actions"><a class="button primary" href="#project">${t.discover} <span aria-hidden="true">↓</span></a><a class="button ghost" href="#news">${t.latest} ${arrow}</a></div></div><div class="hero-visual" aria-hidden="true"><span class="orbit orbit-one"></span><span class="orbit orbit-two"></span><span class="orbit orbit-three"></span><span class="orbit-center">SPORE<span>MP</span></span>${t.stages.map((stage,i)=>`<span class="orbit-stage stage-${i+1}"><small>${String(i+1).padStart(2,'0')}</small>${esc(stage)}${i===1?`<em>${t.firstTests}</em>`:''}</span>`).join('')}</div></div><div class="hero-facts wrap"><span>${t.heroFact1}</span><span>${t.heroFact2}</span><span>${t.heroFact3}</span></div></section>
<div class="development-strip wrap"><span class="strip-label">${t.strip}</span><p><time datetime="${data.news[0].date}">${date(data.news[0].date)}</time> <strong>${esc(data.news[0].title)}</strong></p><a href="./${esc(articlePath(data.news[0]))}">${t.stripLink} ${arrow}</a></div>
